...
export default function InboxView({ messages, onQuickView, searchQuery, onSearchChange }) {
  return (
    <main className="flex flex-col min-h-0 p-4 lg:p-6 lg:pt-6">
      <div className="neo-card rounded-none flex flex-col flex-1 min-h-0 animate-fade-up">
        {/* Header */}
        <div className="flex items-center justify-between px-5 lg:px-6 py-4 neo-divider shrink-0">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-lg font-black uppercase" style={{ fontFamily: "'Space Grotesk', sans-serif", color: 'var(--neo-text)' }}>Inbox</h2>
        ...
        <div className="flex-1 overflow-y-auto">
          {!messages || messages.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-20" style={{ color: 'var(--neo-text-muted)' }}>
            ...
            <div className="divide-y-[3px] divide-[var(--neo-border)]">
              {messages.slice(0, 20).map((msg) => (
                <div
                  key={msg.id}
                  className="msg-item group flex items-start gap-4 px-5 lg:px-6 py-4 neo-row-hover cursor-pointer"
